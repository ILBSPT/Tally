...
int main(int argc, char* argv[]){
    ...
    int number_of_files = 4000; //max 4000
    int number_of_cycles = 100;
    int number_of_metacycles = 1000;

    Minithread m[number_of_threads];
    
    for(int i = 0; i < number_of_threads; i++){
        asprintf(&fOpt.file_name, "/cache_copys/random_walk_%d", (i % number_of_files) + 1);
        m[i] = minithread_init(NULL, 1<<15, NULL, &fOpt, mArray, 1, 100);
        free(fOpt.file_name);
    }
